...
def stole():
    c = 0 # Стоимость украденного 
    for r in range(0, thief_r): 
        b = 0 # Занятость рюкзака
        i = 0
        stolen = []
        for i in stolen: # Проходимся по массиву с украденым и удаляем из основного массива, возможно костыль, но что бы не было путаницы с индексами при переборе
            items.remove(i)
        while b < thief_w and i < len(items): # Начинаем забирать самые ценные предметы
            if b+items[i][0] <= thief_w: # Проверяем что они влезают 
                stolen.append(items[i]) 
                b += items[i][0]
            else:
                i+=1 # Если не можем взять предмет, то проверяем можем ли взять
        for i in stolen: # Считаем сумму
            c+=i[1]
    return c 
